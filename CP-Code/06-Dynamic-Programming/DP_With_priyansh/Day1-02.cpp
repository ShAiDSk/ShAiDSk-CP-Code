...
struct Answer{
    ...
    void Solve(){
        ...
        // ❯ g++ Day1-02.cpp -o Day1-02.out; ./Day1-02.out < in > out; cat cerr.txt; echo -e "\nOutput"; cat out
        Help h;
        // given a 2D grid (n * m), find the path from top left(0, 0) to
            // bottom right(n - 1, m - 1) with minimum sum of values on the path.
        // int n, m; cin >> n >> m;
        for (int i = 0; i < n; i++){
            for (int j = 0; j < m; j++){
                dp[i][j] = -1;
